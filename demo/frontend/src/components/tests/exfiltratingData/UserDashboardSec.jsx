...
const UserDashboardCase3 = () => {
    ...
    const fetchUserDetails = async (username) => {
      try {
        const response = await fetch(
          `http://localhost:8800/api/secure/case3/lookup?username=${username}`,
          {
            credentials: "include",
          }
        );
  ...
  const handleLogout = async () => {
    try {
      const response = await fetch(
        "http://localhost:8800/api/secure/case3/logout",
        {
          method: "POST",
          credentials: "include",
        }
      ...
      if (response.ok) {
        ...
        navigate("/test/exfiltratingData/secure", { replace: true });
      }
    } catch (error) {
